...
class OwnerActions(ProfileActions, OwnerValidation):
    def get_owner_detail(self, email, id):
    ...
    def create_owner_account(self, name, email):
    ...
    def create_owner(self, auth_email, name, email, phone, company, website, address, country, state, district, city, postalcode, aadhar, site_origin):
        ...
        new_user = self.create_owner_account(name, email)
        ...
        ActivityFunctions().save_activity(
            new_user.id, "owner_create", ['Owner', user.email])
    ...
    def get_owner_list(self, email, page, search):
        try:
            page = int(page)
        ...
        owner_list, page_count = self.get_all_owners(page, search)
        context = {
        ...
        # Save Activity
        ActivityFunctions().save_activity(user.id, "owner_list_view", [page])
